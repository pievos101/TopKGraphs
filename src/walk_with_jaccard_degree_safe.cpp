...
IntegerVector walk_with_jaccard_degree_safe_cpp2(
  ...
  for (int i = 1; i <= walk_depth; i++) {
    ...
    // ---- choose ANCHOR from walk history ----
    int anchor;
    if (i > 1) {
      anchor = walk[R::runif(0, i)]; // random past node
    } else {
      anchor = start_node;
    }

    IntegerVector anchor_neighbors = adj_list[anchor - 1];

    // convert anchor neighbors to set (FAST intersection)
    std::unordered_set<int> anchor_set(anchor_neighbors.begin(), anchor_neighbors.end());
    ...
    for (int j = 0; j < m; j++) {
      ...
      int inter_len = 0;
      for (int x : nv) {
        if (anchor_set.count(x)) inter_len++;
      }

      int union_len = nv.size() + anchor_neighbors.size() - inter_len;
